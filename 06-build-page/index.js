...
function createDirectory(struct, filePath) {
  return new Promise((resolve) => {
    const subPromises = [];
    const files = struct.files;
    const subdirs = struct.subdirs;
    const dirPath = struct.name;
    const sourceDirBasename = path.basename(struct.name);
    const newRootPath = path.resolve(filePath, sourceDirBasename);
...
scanDirectory(assetsDir).then(struct => createDirectory(struct, targetDirPath));
